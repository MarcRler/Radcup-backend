...
exports.getStatisticsForPlayer = function(req, res) {
    ...
    findGamesForUsername(function(dataObject) {
        ...
        for (game in dataObject.games) {
            ...
            var game = dataObject.games[game]
            console.log(game);
...
checkWinner = function(statistic, game, username) {

    switch (game.results.winner) {
        case "Red":
            if (game.players.one == username || game.players.two == username) {
                statistic.won += 1;
            } else {
                statistic.lost += 1;
            }
            break;
        case "Green":
            if (game.players.three == username || game.players.four == username) {
                statistic.won += 1;
            } else {
                statistic.lost += 1;
            }
